...
# 기본 설정
# =========================================================
st.set_page_config(page_title="엔진 결함 QC 모니터링", layout="wide")
EPS = 1e-9
...
def render_fault_factor_panel(fault_text: str):
    ...
    st.markdown('<div class="card" style="padding-top:10px;"><div class="panel-title" style="margin-bottom:10px;">추정 결함 요인</div>', unsafe_allow_html=True)
    st.markdown(
        f"""
        <div style="font-size:15px; color:#c9d4df; line-height:1.75; margin-top:8px; margin-bottom:14px;">
...
def make_radar_compare_chart(current_row: pd.Series, normal_df: pd.DataFrame, cols: list[str], full_df: pd.DataFrame):
    ...
    fig.update_layout(
        polar=dict(
            ...
            angularaxis=dict(tickfont=dict(size=11, color='#e8edf3'), gridcolor='#2a3646')
        ),
        height=520,
        margin=dict(l=50, r=10, t=50, b=20),
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
...
# =========================================================
st.markdown(
    "<h1 style='margin-top:25px;'>엔진 결함 QC 모니터링</h1>",
    unsafe_allow_html=True
)
...
st.markdown("<hr style='border:1px solid #2a3646; margin:6px 0 14px 0;'>", unsafe_allow_html=True)
